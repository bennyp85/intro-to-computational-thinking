...
student = (name = "wifter smit", kerberos_id = "wifter")
...
function demean(xs)
    μ = mean(xs)  # Calculate the mean
    demeaned_xs = xs .- μ # Create a new array with demeaned values
    return demeaned_xs
...
# ╔═╡ e20b27bc-88f4-4536-9a68-f42905900191
begin
	function get_blue(pixel::AbstractRGB)
		blue_channel = pixel.b
		return blue_channel
	end
...
function mean_color(image)
	return RGB(sum(get_reds(image))/length(get_reds(image)),
	sum(get_greens(image))/length(get_greens(image)),
	sum(get_blues(image))/length(get_blues(image)))
end
...
function noisify(color::AbstractRGB, s)
	red_noise = noisify(color.r, s)
	green_noise = noisify(color.g, s)
	blue_noise = noisify(color.b, s)
	return RGB(red_noise, green_noise, blue_noise)
end
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "65f1020a59325c778b6f1a59c9bea7bbc6ac5aa2"
